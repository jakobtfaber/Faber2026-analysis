#!/usr/bin/env python3
"""Run reference-parity independent and joint CHIME/DSA DM fits."""
...
def fit_band(row: dict, data_dir: Path, absolute_grid: np.ndarray) -> dict:
...
def run(manifest: Path, data_dir: Path, output: Path) -> list[dict]:
    ...
    for burst in sorted(by_event):
        ...
        chime = fit_band(pair["chime"], data_dir, grid)
        dsa = fit_band(pair["dsa"], data_dir, grid)
...
def main() -> None:
    ...
    parser.add_argument(
        "--data-dir",
        type=Path,
        default=Path("~/Data/Faber2026/dsa110/DSA_bursts").expanduser(),
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    if args.recompute_joint_only:
        ...
        run(args.manifest, args.data_dir, args.output)
