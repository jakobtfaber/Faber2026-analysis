...
**Compiled:** 2026-07-09 &nbsp;·&nbsp; **Referee report:** `docs/referee_report_2026-07-07.md` &nbsp;·&nbsp; **origin/main:** `bf0f902`
...
**Summary:** of 27 referee items, **18 are fully resolved and merged**; the remaining 9 are blocked on either (a) the pending scattering/scintillation sections the referee itself flagged as to-be-added, (b) author-only design decisions (D2–D5), or (c) submission-time actions (Zenodo DOI mint, abstract-slot fill, citation-status watch).
...
| ✅ | **B4** | Timing residuals: errors, acceptance criterion, sign convention, geometric range | DONE | toa.tex: geometric delay + sign convention (L23-31), Residuals & systematics subsection (L94), |tau_geo| bound (handoff 07-08-07-29) |
...
- **D2–D5 (design decisions).** These lock choices the per-sightline attribution ledger inherits (closure-regime column, sub-band diagnostic labeling, scintillation double-use, energetics comparability). They require author judgement, not code.
- **Scattering / scintillation / turbulence / energetics sections.** Board phases V/A/B/C/D: the fit re-validation ladder, per-sightline geometry-selection campaign, and two-screen analysis on joint CHIME+DSA scintillation. These are new data analysis on real burst products — the paper's central contribution — and cannot be written without running that campaign.
- **Submission-time actions.** B5/M11 Zenodo DOI mint, M16 abstract-slot fill from frozen B2 numbers, MW4 NE2025 publication-status watch. Each is a one-step author action at submission, not a code change.
