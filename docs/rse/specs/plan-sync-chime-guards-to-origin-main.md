...
**Status:** Draft
**Related Documents:**
- [Assessment: ChatGPT scint de-combing review](assessment-chatgpt-scint-decombing-review.md) *(untracked; committed by this plan)*
- [Experiment: freya CHIME instrumental origin](experiment-freya-chime-instrumental-origin.md)
...
- `docs/rse/specs/assessment-chatgpt-scint-decombing-review.md` untracked
...
**Tasks:**
- [ ] **Branch off up-to-date main; carry ONLY this lane's paths**
  ...
  git add pipeline docs/rse/specs/assessment-chatgpt-scint-decombing-review.md docs/rse/specs/plan-sync-chime-guards-to-origin-main.md docs/rse/journal.jsonl
...
- [ ] **Edit `REPRODUCE.md:27-34`** — replace the stale paragraph
...
- [ ] **Edit `REPRODUCE.md:150-158`** — extend the pin history sentence.
...
- [ ] **Edit `repro_manifest.csv`** — append to the `notes` field of each of
...
- [ ] **Gate: parity byte-identity across the bump** (the #71 evidence check;
...
- [ ] **Append a journal entry, commit, [ONE-WAY] push + PR**
  ...
  git commit -m "Bump pipeline pin 79eaf7e -> ${PIN2:0:7} (CHIME artifact-control guards)" \
    ...
    -- pipeline docs/rse/specs/assessment-chatgpt-scint-decombing-review.md docs/rse/specs/plan-sync-chime-guards-to-origin-main.md docs/rse/journal.jsonl REPRODUCE.md repro_manifest.csv
...
- [ ] **Wait for the required `parity` check, then [ONE-WAY] merge**
...
**Verification:**
- [ ] `git fetch origin main --quiet && git ls-tree origin/main pipeline` shows `$PIN2`.
- [ ] `git show origin/main:docs/rse/specs/assessment-chatgpt-scint-decombing-review.md | head -1` → `# Assessment: ChatGPT review of the scintillation "de-combing" code`.
...
**Tasks:**
- [ ] **Reconcile local main** (manuscript lane rides untouched)
...
- [ ] **Fresh-clone reproducibility test (the real gate the old convention
  failed silently)**
...
- [ ] `gh pr view $FLITS_PR --repo jakobtfaber/dsa110-FLITS --json state --jq .state` → `MERGED`
- [ ] `git -C pipeline ls-remote origin pin/faber2026` → `$PIN2`
- [ ] `git -C pipeline cat-file -e origin/main:scintillation/scint_analysis/chime_artifact_guards.py` → exit 0
- [ ] `git ls-tree origin/main pipeline` (Faber2026) → `$PIN2`
...
- [ ] `docs/rse/specs/assessment-chatgpt-scint-decombing-review.md` — committed
      as-is (Phase 3).
...
- [assessment-chatgpt-scint-decombing-review.md](assessment-chatgpt-scint-decombing-review.md)
