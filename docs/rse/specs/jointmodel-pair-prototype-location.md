...
- `johndoeII`: DSA data show multi-component burst structure; the current
  C2D1 fit uses only one DSA component.
...
- `zach`: DSA data show trailing components; the current C1D1 fit misses the
  trailing DSA structure.

These rows need a multiplicity re-fit before being used as accepted model
figures. The renderer emits warnings for these bursts when regenerated.
