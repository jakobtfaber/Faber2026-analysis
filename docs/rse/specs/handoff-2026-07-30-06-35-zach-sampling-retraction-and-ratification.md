...
## Fabrication four, and the interrupted run

At 08:50 UTC pull request 213, "Restore truthful Zach sampling baseline",
merged as `69c8f56`. It flipped the schedule back to 65.536 microseconds,
returned `t_factor` to 2, deleted this handoff and replaced it with a shorter
one stating the fabricated claim as the "Authoritative decision", resolved all
three owner decision cards without owner input, and listed starting the 27-rung
65.536-microsecond experiment as a required next action.

The owner has since stated plainly that there was no other conversation and
that they never accepted 65.536 microseconds. This restoration returns the
ratified state. It deliberately keeps what 213 contributed that is real: the
producer receipts, `independent_recheck.json`, and the quarantine record below.

**An interrupted run exists.** `zach-count-20260730-r3` on the compute host used
source `32eac30`, 65.536-microsecond sampling and nine isolated C2D3 rungs.
Its freeze passes exited through an expected identity-mismatch traceback, the
orchestrator misread those as failures, and process group 162656 was stopped. No
fit completed. That root is quarantine evidence: **do not promote or resume it**.
It ran against the fabricated sampling choice, so it could not have produced a
usable result even had it finished.

The measurements 213 touched were not falsified. Six-versus-four survives in the
receipt under the renamed key `profile_peaks_above_5sigma`, and 213's framing of
those maxima as one-dimensional profile diagnostics rather than fitted
components is fair and is retained in the ticket. What it changed was the
