...
| Thread | Verdict | Gate |
|---|---|---|
| 1. RM → intervening cluster/CGM B-field | Chase (strongest; cross-paper seam). DM_int inputs are V4/V5-cleared; RM comes from the polarization companion. **EXECUTED 2026-07-17 (sanctioned); Undermind sweep re-ran the frozen rule; marginal branch adjudicated (owner-delegated): classification MATERIAL at the boundary** — six studies / five systems (Fornax one-vote), RM_lit(obs)=18.9 ≥ 18.6 (1.7% margin, recorded); ±0.5 μG at 0.83 R500 shifts their RM_host by ~7σ (median-independent); memo + ISSUE-010 issued and updated (`memo-thread1-rm-repartition-2026-07-17.md`, plan v1.3). | Phase 4 (prose, both papers) gated on the both-owners agreement — companion-lead decision pending. |
