...
- **Only α ≥ 4 is reachable** through the β co-model's thin-screen closure — fine for freya (the only single-component sightline whose deprecated exp-era fit *suggested* α > 4; that suggestion is the hypothesis the real-data β fit tests, not established physics). Extended-medium (β > 4) branch is an open `@decision`, out of scope. *(Wording corrected 2026-07-02 per maintainer: epistemic status made explicit.)*
