...
**Validation Reports:**
- [validation-harmonic-mask-chime-sweep.md](../validation/validation-harmonic-mask-chime-sweep.md) — the full A/B table, verification detail, caveats, recommendations

**Experiment Reports:**
- [experiment-freya-chime-instrumental-origin.md](../experiment/experiment-freya-chime-instrumental-origin.md) — arm-B1 section now carries the config-default status note (this session)

**Previous Handoffs (same lane):**
- [handoff-2026-07-05-23-24-flits-130-harmonic-mask-merge.md](../handoff/handoff-2026-07-05-23-24-flits-130-harmonic-mask-merge.md) — the specification this session executed (its item 2)

## Critical References

- `docs/rse/specs/validation/validation-harmonic-mask-chime-sweep.md` — all numbers, repro commands, and caveats; read this first
- `docs/rse/specs/handoff/handoff-2026-07-05-23-24-flits-130-harmonic-mask-merge.md` — remaining open items 1 and 3 defined there
...
- `docs/rse/specs/validation/validation-harmonic-mask-chime-sweep.md` — new (commit `d3f27d3`)
- `docs/rse/specs/experiment/experiment-freya-chime-instrumental-origin.md:147-155` — B1 status-update bullet: mask config-default since FLITS #130, oracle reproduced at pin `028fa7c` (same commit)
...
3. [x] **Done (2026-07-06, follow-up session, owner-directed for the whole sample):** all five defect-carrying targets (casey, whitney, phineas, mahi, isha) regenerated on h17 with the `--no-time-shift` de-chirp recipe (isha `--run-unresolvable`, upper-bound-only) and packaged to aligned npz via `build_npz_aligned_generic_20260706.py` (byte-oracled against freya gen-3). Gen-1 products quarantined h17+local, never deleted. Full md5 table + alignment verdicts: `~/Data/Faber2026/dsa110/upchan_codetections/PROVENANCE.md` (2026-07-06 campaign section). Fresh gen-2 casey numbers (mask on): 33.13±39.70 / 95.96±41.75 kHz, modulation indices now physical — still non-detections. Open residue: whitney/phineas/mahi/isha burst configs (FLITS lane); six never-generated co-detections need per-sightline U sizing; pinned `pipeline/scintillation/DATA_PROVENANCE.md` still lists the gen-1 casey npz md5-era state and the stale `/data/jfaber` path — fix belongs in a FLITS PR, not the pinned submodule.
