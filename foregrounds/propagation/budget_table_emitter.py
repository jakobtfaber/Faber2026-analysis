...
Usage
-----
    from foregrounds.propagation.budget_table_emitter import format_budget_table_tex
    tex = format_budget_table_tex()          # -> full .tex as a string

CLI (also wired as ``--emit-tex`` on the sightline_budget entry point):
    python -m foregrounds.propagation.budget_table_emitter            # writes exports/budget_table.tex
    python -m foregrounds.propagation.budget_table_emitter --check    # parity check, non-zero exit on drift
"""
...
from foregrounds.paths import BUDGET_DATA, RESULTS_ROOT

HERE = pathlib.Path(__file__).resolve().parent
DATA_PATH = BUDGET_DATA
EXPORT_PATH = RESULTS_ROOT / "generated" / "budget_table.tex"
