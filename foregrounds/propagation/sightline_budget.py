...
def _is_placeholder_z(z: float | None) -> bool:
    value = _f(z)
    return not math.isfinite(value) or abs(value - PLACEHOLDER_Z) < 1.0e-6
...
def _lookup_tau_fit(name: str, bursts_dir: str | None) -> dict | None:
    ...
    """
    from foregrounds.propagation.tau_consistency import load_allexp_joint_tau_for_budget
...
def foreground_unified(
    ...
    *,
    ...
    (foregrounds/visualization/systems_figures.py) -- so figure annotations cannot drift
    ...
    if use_registry:
        from foregrounds.census.census_registry import (
...
def build_sightline_budget(
    ...
    *,
    ...
    if z_is_placeholder:
        verdict_dm = "host redshift is missing or placeholder; cosmic and host DM budget unavailable"
    else:
    ...
    flags = {
        "z_frb": "MISSING_OR_PLACEHOLDER" if z_is_placeholder else "MEASURED",
        ...
        "dm_mw_halo": "PRIOR",
        "dm_cosmic": (
            "NOT_AVAILABLE (missing or placeholder redshift)"
            if z_is_placeholder
            else "PREDICTED_MEAN"
        ),
...
def main(argv: list[str] | None = None):
    ...
    parser.add_argument(
        "--configs-dir",
        default=os.path.join(base_dir, "config", "fits", "scattering", "bursts", "chime"),
    )
    parser.add_argument("--bursts-dir")
    args = parser.parse_args(argv)
    results_dir = os.path.abspath(args.results_dir)
    configs_dir = os.path.abspath(args.configs_dir)
    bursts_dir = os.path.abspath(args.bursts_dir) if args.bursts_dir else os.path.join(
        results_dir, "bursts"
    )
...
def format_budget_table_tex(data_path=None) -> str:
    ...
    canonical import path is ``foregrounds.propagation.sightline_budget`` (adjacent to
    ...
    """
    from foregrounds.propagation.budget_table_emitter import format_budget_table_tex as _emit
