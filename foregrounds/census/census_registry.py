...
def scratch_codetection_dir(explicit: Path | str | None = None) -> Path:
    ...
    env = os.environ.get("FABER2026_FOREGROUND_SCRATCH")
    if env:
        return Path(env)
...
def budget_eligible(final_verdict: str, obj_type: str, b_over_r500: float) -> bool:
    """Separate census confirmation from physical budget eligibility."""
...
def recompute_impact_kpc(
    ...
    (fiducial cosmology of foregrounds.census.config) instead.
    """
