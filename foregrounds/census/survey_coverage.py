...
def survey_in_footprint(survey_key: str, coord: SkyCoord) -> bool | None:
    ...
    if survey_key in FOOTPRINT_RULES and FOOTPRINT_RULES[survey_key] != "all_sky":
        try:
            ...
            from foregrounds.census.survey_footprint_mocs import CDS_MOC_IDS, load_survey_moc
...
def engine_survey_key(engine: Any) -> str:
    ...
    from foregrounds.census.engines import VizierEngine
    from foregrounds.census.engines_extra import (
        ClusterEngine,
        DesiDr1Engine,
    ...
    if isinstance(engine, VizierEngine):
        from foregrounds.census.config import VIZIER_CATALOGS
