...
| `../../../../.archive/outdated-science/2026-07-17/foregrounds/census/data/sightline_attribution_matrix.csv` | Archived historical two-screen × foreground cross-check; not current science |

Regenerate:

```bash
python -m foregrounds.visualization.build_artifacts
```

α=4 joint refits (needs FLITS_RUNS data on HPC):

```bash
python -m foregrounds.propagation.run_tau_consistency_refits casey
```
