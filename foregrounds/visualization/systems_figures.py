...
Cluster inputs are read directly from the authoritative census registry.
"""
...
try:
    ...
    from foregrounds.propagation import scattering_predict as scat
    from foregrounds.propagation.sightline_budget import INTERIOR_B_OVER_RVIR
...
def load_cluster_targets(census_data_dir: str, n: int = 4) -> list[tuple]:
    """Load the innermost Phineas clusters from the authoritative registry."""
    registry = pd.read_csv(
        os.path.join(census_data_dir, "intervening_census_registry.csv")
    )
    rows = registry[
        (registry["nickname"] == "phineas") & (registry["type"] == "cluster")
    ].copy()
    columns = ("impact_kpc", "b_over_r500", "m500_1e14msun", "r500_mpc", "best_z")
    for column in columns:
        rows[column] = pd.to_numeric(rows[column], errors="coerce")
    rows = rows.dropna(subset=list(columns))
    rows = rows[(rows[list(columns)] > 0).all(axis=1)]
    rows = rows.sort_values(["b_over_r500", "obj"]).head(n)
    if len(rows) != n:
        raise ValueError(f"expected {n} valid Phineas clusters, found {len(rows)}")
    return [
        (
            str(row.obj).split(",", 1)[0],
            float(row.impact_kpc),
            float(row.b_over_r500),
            float(row.best_z),
            float(row.m500_1e14msun) * 1.0e14,
            float(row.r500_mpc) * 1.0e3,
        )
...
def cluster_params(
    b_kpc: float, b_over_r500: float, z: float, m500: float, r500: float
) -> dict:
    """Compute the mNFW column from catalog mass and geometry."""
    m200 = scat.m200_from_m500_nfw(m500, z)
    return {
        "r500_kpc": r500,
        "m500_msun": m500,
        "m200_msun": m200,
...
def make_cluster_figure(cluster_targets: list[tuple]):
...
def _selfcheck(
    ...
    for objid, b_kpc, b_over_r500, z, m500, r500 in cluster_targets[2:]:
        assert cluster_params(b_kpc, b_over_r500, z, m500, r500)["dm_at_b"] == 0.0, (
...
def main():
    ...
    cluster_targets = load_cluster_targets(args.census_data_dir)
    _selfcheck(targets, cluster_targets, args.results_dir, args.census_data_dir)
    ...
    for fig, stem in (
        ...
        (make_cluster_figure(cluster_targets), "clusters_icm"),
    ):
