...
def _write_chime_fixture(path: Path) -> None:
    ...
    seconds = 1_700_000_000.0 + fpga_count * delta_time_s
    dtype = np.dtype(
        [("ctime", "<i8"), ("ctime_offset", "<f8"), ("fpga_count", "<u8")]
    )
    time0 = np.zeros(frequency_mhz.size, dtype=dtype)
    time0["ctime"] = np.floor(seconds).astype(np.int64)
    time0["ctime_offset"] = seconds - time0["ctime"]
    time0["fpga_count"] = fpga_count.astype(np.uint64)
    event_unix_s = float(seconds.max() + 0.01)
...
def _install_fake_blimpy(monkeypatch: pytest.MonkeyPatch, *, tstart: float = 60_000.0) -> None:
    class FakeWaterfall:
        def __init__(self, _path: str, *, load_data: bool) -> None:
            assert load_data is False
...
def _receipt_fixture(
    ...
    paths = {
        name: tmp_path / name
        for name in ("raw_chime_h5", "raw_dsa_filterbank", "trigger_recovery")
    }
...
def test_receipt_refuses_accidental_fit_eligibility(
    ...
    monkeypatch.setattr(
        timing,
        "audit_dsa",
        lambda _path, _entry: {"fit_observation_time_origin_eligible": True},
    )
...
def test_receipt_rejects_missing_or_wrong_sample_zero_hold(
    ...
    monkeypatch.setattr(timing, "audit_dsa", lambda _path, _entry: dsa)
    output = tmp_path / "timing-receipt.json"
    sentinel = b"preserved receipt sentinel\n"
    output.write_bytes(sentinel)
    monkeypatch.setattr(
...
def test_atomic_publication_failure_preserves_existing_receipt_and_cleans_temp(
    ...
    sentinel = b"preserved receipt sentinel\n"
    output.write_bytes(sentinel)
    monkeypatch.setattr(timing, "build_receipt", lambda _path: {"status": "blocked"})
...
def test_atomic_publication_writes_complete_receipt(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(timing, "build_receipt", lambda _path: receipt)
    timing.publish_receipt(tmp_path / "config.json", output)
