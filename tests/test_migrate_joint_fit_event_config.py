...
@pytest.mark.parametrize("event", ["oran", "isha"])
def test_checked_in_migration_is_canonical_and_schema_valid(event: str) -> None:
    config_path = ROOT / "analysis-configs" / "absolute-dm" / f"{event}.json"
    ...
    assert config["event_binding_sha256"] == event_binding_sha256(config)
