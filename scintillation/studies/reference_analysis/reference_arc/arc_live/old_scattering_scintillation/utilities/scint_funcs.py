...
def acf_per_subband(spec,freqs,freqids,num_subbands=2,savefig='./acf_per_freq.pdf',plot_fit=True,maxlag=None,snsubband=False,offspec=None):
    ...
    # mask all the 0s and nan values in spec
    # remove me
    # Convert `spec` into a masked array if it's not already
    spec = np.ma.masked_where((spec == 0) | np.isnan(spec), spec)

    # If `offspec` is provided, mask it at the same locations as `spec`
    if offspec is not None:
        offspec = np.ma.array(offspec, mask=spec.mask)
    ##################################################################
