...
candidate grid spanning the exp-era suggestion, and record the verdicts:
    ...
    conda run -n flits python analysis/scattering/studies/legacy-joint-refits/tail_coverage.py
"""
...
def _load_driver():
    ...
    path = REPO / "scattering" / "studies" / "legacy-joint-refits" / "local_runs" / "run_joint_fit.py"
...
def run_preflight(burst: str = "freya", min_efolds: float = DEFAULT_MIN_EFOLDS) -> dict:
    ...
    """
    local_runs = REPO / "scattering" / "studies" / "legacy-joint-refits" / "local_runs"
    driver = _load_driver()
