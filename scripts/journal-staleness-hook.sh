...
if [ "$age" -ge 10 ]; then
  printf '{"hookSpecificOutput":{"hookEventName":"UserPromptSubmit","additionalContext":"[journal] Last entry %sm ago — 10-min cadence lapsed. Append: scripts/journal-append.sh <agent> <lane> <state> \\"<note>\\", then rebake + redeploy the board (docs/rse/journal-protocol.md)."}}\n' "$age"
fi
exit 0
