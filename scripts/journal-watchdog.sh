...
"$REPO/scripts/journal-append.sh" "watchdog/launchd" "repo" "info" \
  "unattributed activity: ${n}+ file(s) touched in last 10m ($top) while journal stale ${age}m - active agent must self-report (docs/rse/journal-protocol.md)"
