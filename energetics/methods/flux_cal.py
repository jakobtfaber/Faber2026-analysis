...
def burst_epoch_position(nick):
    ...
    b = yaml.safe_load(
        (Path(__file__).resolve().parents[2] / "config" / "bursts.yaml").read_text()
    )
    e = b["bursts"][nick]
...
def dsa_band_fluence_jy_ms_hz(nick):
    ...
    """
    from energetics.methods.dsa_beam import beam_gain
...
def joint_band_fluence_jy_ms_hz(nick, band):
    ...
    if band == "D":
        ...
        from energetics.methods.chime_beam import chime_sigma_jy, load_chime_sefd
