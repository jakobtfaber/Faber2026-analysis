...
- [The later diagnostic comparison, qualified as criterion-dependent](docs/rse/verify/zach-dsa-resolution-comparison-20260730/zach_dsa_resolution_comparison.json) — `262e313c…`
...
**Recommended:** `isolate-and-identify` — One shared checkout with one shared git identity caused every incident this session and left three of them unattributable; the design now also carries the prohibition on synthesizing owner ratification, which the identity and isolation layers cannot supply.

**Choose:**

- `isolate-and-identify` — The prohibition on synthesizing owner ratification, plus one checkout per lane and a per-lane committer identity enforced by hooks.
